...
describe('consul', () => {
  ...
  beforeEach(() => {
    ...
    mockConsul = {
      session: {
        ...
        renew: jest.fn().mockResolvedValue([{ CreateIndex: 1, ModifyIndex: 2 }]),
  ...
  describe('configureHostSession', () => {
    it('should create a new session', async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining(mockSession));
    });
    ...
    it('should warn and wait if there is an existing session lock', async () => {
      ...
      mockConsul.session.node = jest.fn()
  ...
  describe('getPods', () => {
    it('should return empty array when no pods exist', async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith({ keys: [] }, expect.any(String));
    });
    
    it('should fetch and parse pod entries', async () => {
      // Arrange
      const podKeys = ['raftainer/pods/configs/pod1', 'raftainer/pods/configs/pod2'];
  ...
  describe('tryLockPod', () => {
    ...
    it('should return null if unable to lock any instance', async () => {
      ...
      // Act
      const result = await tryLockPod(mockConsul, mockSession, podLocks, mockPod);
    ...
    it('should skip existing lock key if it would violate maxInstances', async () => {
      ...
      mockConsul.kv.set
        .mockResolvedValueOnce(true); // First regular attempt succeeds
  ...
  describe('deregisterServices', () => {
    it('should deregister services not in activeServiceIds', async () => {
      ...
      expect(mockConsul.agent.service.deregister).toHaveBeenCalledWith('service2');
      expect(logger.info).toHaveBeenCalledWith(
        expect.objectContaining({
