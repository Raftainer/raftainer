...
export class ConstraintMatcher {
  ...
  private async meetsGpuConstraints(pod: ConsulPodEntry) {
    try {
      const { controllers: gpus }: Systeminformation.GraphicsData =
        await si.graphics();
      logger.debug(
        {
      ...
      const gpuConstraints = pod.pod.containers.flatMap(
        (container) => container.hardwareConstraints?.gpus || [],
      );
      for (const constraint of gpuConstraints) {
        ...
        if (
          constraint.vramBytes &&
          !gpus.some((gpu) => (gpu.vram || 0) >= (constraint.vramBytes || 0))
        ) {
          logger.debug(
            {
              podName: pod.pod.name,
  ...
  async meetsConstraints(pod: ConsulPodEntry) {
    try {
      ...
      logger.debug(
        { podName: pod.pod.name, result },
        "Hardware constraint check result",
      );
      return result;
    } catch (error) {
      logger.error(
